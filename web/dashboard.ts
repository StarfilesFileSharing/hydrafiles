...
try {
	...
	const tickHandler = async () => {
		...
		(document.getElementById("knownFiles") as HTMLElement).innerHTML = `${await window.hydrafiles.files.db.count()} (${Math.round((100 * (await window.hydrafiles.files.db.sum("size"))) / 1024 / 1024 / 1024) / 100}GB)`,
			(document.getElementById("storedFiles") as HTMLElement).innerHTML = `${(await window.hydrafiles.fs.readDir("files/")).length} (${Math.round((100 * await window.hydrafiles.utils.calculateUsedStorage()) / 1024 / 1024 / 1024) / 100}GB)`,
			populateTable();
