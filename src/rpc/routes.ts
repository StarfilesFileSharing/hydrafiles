...
router.set("/endpoint", async (req, headers, client) => {
  ...
  if (hostname === `${base32Encode(new TextEncoder().encode(pubKey.x)).toLowerCase().replace(/=+$/, "")}.${base32Encode(new TextEncoder().encode(pubKey.y)).toLowerCase().replace(/=+$/, "")}`) {
    const body = client.config.reverseProxy
      ? await (await fetch(client.config.reverseProxy)).text()
      : (typeof client.handleCustomRequest === "undefined" ? "Hello World!" : await client.handleCustomRequest(new Request(`hydra://${hostname}/`)));
