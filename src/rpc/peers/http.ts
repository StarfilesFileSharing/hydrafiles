...
export default class HTTPServer {
	...
	private async listen(): Promise<void> {
		...
		if (certFile instanceof Error) Utils.error(certFile);
		else if (keyFile instanceof Error) Utils.error(keyFile);
		else {
	...
	private onListen = async (hostname: string, port: number): Promise<void> => {
		...
		if (!file) return;
		if (!(await file.download())) Utils.error("RPC:      Download test failed, cannot connect to network");
		else {
			Utils.log("RPC:      Connected to network");
			if (Utils.isIp(RPCPeers._client.config.publicHostname) && Utils.isPrivateIP(RPCPeers._client.config.publicHostname)) Utils.error("Public hostname is a private IP address, cannot announce to other nodes");
			else {
				...
				if (self instanceof ErrorNotFound) Utils.error("HTTP:     Failed to find self in peers");
				else {
					const response = await self.downloadFile(file);
					if (response instanceof ErrorDownloadFailed) Utils.error("HTTP:      Failed to download file from self");
					else {
