...
class Utils {
  ...
  hasSufficientMemory = (fileSize: number): boolean => os.freemem() > (fileSize + this.config.memory_threshold)
  promiseWithTimeout = async <T>(promise: Promise<T>, timeoutDuration: number): Promise<T> => await Promise.race([promise, new Promise<never>((_resolve, reject) => setTimeout(() => reject(new Error('Promise timed out')), timeoutDuration))])

  promiseWrapper = (promise: Promise<any>): { promise: Promise<any>, isFulfilled: boolean } => {
  ...
  bufferToStream (arrayBuffer: ArrayBuffer): Readable {
